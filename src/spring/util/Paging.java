...
public class Paging {
	...
	public Paging(int nowPage, int rowTotal, 
		...
		//크다면 전체 페이지값을 현재 페이지 값으로 지정
		if(nowPage > totalPage)
			nowPage = totalPage;
		
		//현재 블럭의 시작페이지 값과 마지막 페이지 값을
		//구한다.
		startPage = 
			(int)((nowPage-1)/blockPage)*blockPage+1;
		endPage = startPage + blockPage - 1;
		
		//마지막페이지의 값이 전체페이지의 값보다
		//크면 마지막페이지의 값을 전체페이지 값으로 지정
		if(endPage > totalPage)
			endPage = totalPage;
		
		//현재 페이지 값에 의해 시작 게시물의 행번호와
		//마지막 게시물의 행번호를 지정하여
		//현재 페이지에 보여질 게시물 목록을 얻을
		//준비를 하자!
		...
		//마지막페이지 값을 알았으니 페이징 기법에 
		//사용할 코드를 작성하여 StringBuffer에 저장하자!
