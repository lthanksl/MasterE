 ...
 /*, datatype : "json" */
	...
	$(document).ready(function () {
		...
		jQuery("#gridMain").jqGrid({
			...
			colNames: ['employee_id','first_name','department_id','job_id'
				/* , '대분류', '대분류명', '중분류', '중분류명', '소분류', '소분류명', '부하코드', '부하코드명', '단가', '월', '시', '등록일자', '수정일자'
				 */

			],
			colModel: [
					{ name: 'employee_id', index: 'employee_id', width: 0, sorttype: "text" },
					{ name: 'first_name', index: 'first_name', width: 0, sorttype: "text" },
					{ name: 'department_id', index: 'department_id', width: 0, sorttype: "text" },
					{ name: 'job_id', index: 'job_id', width: 0, sorttype: "text" },
